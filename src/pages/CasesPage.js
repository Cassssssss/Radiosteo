  ...
  return (
    <S.CaseCard>
      <Link to={`/radiology-viewer/${cas._id}`}>
        <S.CaseImage 
          src={cas.mainImage ? `http://localhost:5002/${cas.mainImage}` : 
              (cas.folders && cas.folders[0] && cas.folderMainImages && cas.folderMainImages[cas.folders[0]]) ? 
              `http://localhost:5002/${cas.folderMainImages[cas.folders[0]]}` : 
...
function CasesPage() {
  ...
  const handleImageClick = useCallback((folder, index) => {
    setSelectedImage(`http://localhost:5002/${selectedCase.images[folder][index]}`);
  ...
  const navigateImage = useCallback((direction) => {
    ...
    setSelectedImage(`http://localhost:5002/${images[newIndex]}`);
  ...
  const addNewFolder = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.post(`${API_BASE_URL}/${selectedCase._id}/folders`, { folderName: newFolderName });
    ...
    try {
      for (const [folder, images] of Object.entries(newImages)) {
        for (const image of images) {
          ...
          await axios.post(`${API_BASE_URL}/${selectedCase._id}/images`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
          });
        }
      }
      const updatedCase = await axios.get(`${API_BASE_URL}/${selectedCase._id}`);
      ...
      console.error('Erreur lors de l\'ajout des images au cas:', error);
    } finally {
      setIsLoading(false);
    }
  ...
  const setMainImage = useCallback(async (event) => {
    ...
    if (file && selectedCase) {
      ...
      try {
        ...
        const response = await axios.post(`${API_BASE_URL}/${selectedCase._id}/main-image`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setCases(prevCases => prevCases.map(c => c._id === selectedCase._id ? response.data : c));
        setSelectedCase(response.data);
  ...
  const setFolderMainImage = useCallback(async (event, folder) => {
    ...
    if (file && selectedCase) {
      ...
      try {
        ...
        formData.append('folder', folder);
        const response = await axios.post(`${API_BASE_URL}/${selectedCase._id}/folder-main-image`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setCases(prevCases => prevCases.map(c => c._id === selectedCase._id ? response.data : c));
        setSelectedCase(response.data);
  ...
  const deleteFolder = useCallback(async (folder) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer le dossier "${folder}" ?`)) {
      try {
        await axios.delete(`${API_BASE_URL}/${selectedCase._id}/folders/${folder}`);
        const updatedCase = await axios.get(`${API_BASE_URL}/${selectedCase._id}`);
        setCases(prevCases => prevCases.map(c => c._id === selectedCase._id ? updatedCase.data : c));
        setSelectedCase(prevCase => ({
          ...prevCase,
          folders: prevCase.folders.filter(f => f !== folder),
          images: Object.fromEntries(Object.entries(prevCase.images).filter(([key]) => key !== folder)),
          folderMainImages: Object.fromEntries(Object.entries(prevCase.folderMainImages || {}).filter(([key]) => key !== folder))
        }));
      } catch (error) {
  ...
  const verifyImages = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${API_BASE_URL}/${selectedCase._id}/images`);
  ...
  const deleteExistingImage = useCallback(async (folder, imagePath) => {
    try {
      await axios.delete(`${API_BASE_URL}/${selectedCase._id}/images`, {
  ...
  const updateCaseDifficulty = useCallback(async (id, difficulty) => {
    try {
      await axios.patch(`${API_BASE_URL}/${id}`, { difficulty });
  ...
  const updateCaseAnswer = useCallback(async (id, answer) => {
    try {
      await axios.patch(`${API_BASE_URL}/${id}`, { answer });
      setCases(prevCases => prevCases.map(c => 
        c._id === id ? { ...c, answer } : c
      ));
    } catch (error) {
  ...
  const handleAddTag = useCallback(async (caseId, tag) => {
    try {
      const response = await axios.patch(`${API_BASE_URL}/${caseId}/tags`, { tagToAdd: tag });
  ...
  const handleRemoveTag = useCallback(async (caseId, tagToRemove) => {
    try {
      const response = await axios.patch(`${API_BASE_URL}/${caseId}/tags`, { tagToRemove });
      setCases(prevCases => prevCases.map(c => c._id === caseId ? response.data : c));
    } catch (error) {
